...
class tiledmap:
    ...
    def modify_layer(self, location, tile_id, layer_id=0):
        ...
        row, column = location
        if layer_id != self.tilemap["object_layer"]:
    ...
    def save(self, file_path, file_name):
        ...
        pass
    def pygame_get_sprites(self, render_size=1):
    ...
    def pygame_render_chunk(self, chunk_location, render_size=1, lighting=False):
        ...
        surface = pygame.Surface((
            ...
            self.tile_size[1]*self.chunk_size[1]*render_size
        ))
    ...
    def pygame_render_map(self, render_size=1, lighting=False):
        ...
        for layer in range(len(self.tilemap["map_contents"])):
            if layer != self.tilemap["object_layer"]:
                surface.blit(
                    self.pygame_render_layer(layer, render_size=render_size),
                    (0,0)
                )
        return surface